...
+ src文件夹
  ...
  + components文件夹： 一些可以复用的组件存放在这里
  + router文件夹：存放VueRouter路由信息
  + store文件夹：存放Vuex状态
  + views文件夹： 页面视图vue文件存放在这里
  - App.vue  
  - main.ts 项目入口文件
...
*** 组件服务
**** 动态组件(component)
+ 在某些场景,往往需要动态切换页面部分区域的视图，这时内置组件component就显得尤为重要
+ component接收一个名为is的属性，is的值应为在父组件中注册过的组件名称
  #+BEGIN_SRC html
    <style>
      .tabs {
        margin: 0;
  ...
  #+END_SRC
**** 插槽(slot)
+ 通过props选项，组件可以接收多态的数据，如果希望组件也能接收多态的DOM结构，可以使用内置组件slot(插槽)分发内容
+ 定义多个插槽时，可以使用name属性对其进行区分，如果没有指定name属性，则vue将会所有的插槽内容置于默认插槽default中
  #+BEGIN_SRC html
  <div id="app">
    <slot-test>
      <p>使用插槽分发内容</p>
  ...
  <script>
      let SlotTest = {
          template: '<div>' + '<slot name="header">相当于占位符元素，因此这些文字不会被渲染</slot>' + '<slot></slot>' + '</div>'
      }
      let vm = new Vue ({
          el: '#app',
          components: {SlotTest}
  ...
  #+END_SRC
+ 父组件的元素可以分发到对应的插槽中
+ vue还提供了作用域插槽(slot-scope)，可以用来获取子组件回传的数据，用来在父组件中执行多态的渲染和响应，可以实现表格类的组件
**** 组件的缓存
+ keep-alive是一个抽象组件，它不渲染任何DOM元素，也不会出现在组件结构树中，使用它可以缓存一些非动态的组件实例(没有或不需要数据变化)，以保留组件状态或减少重新渲染的开销
+ keep-alive应出现在组件被移除之后需要再次挂载的地方
*** 过渡效果
**** 单节点过渡
+ vue提供了标签为transition的内置组件，在下列情况中，可以给任何元素和组件添加进入/离开时的过渡动画
  - 元素或组件初始渲染时
  - 元素或组件显示/隐藏时(v-if和v-show)
  - 元素或组件切换时
+ vue允许用户使用CSS和JS两种方式来定义过渡效果
  - 使用CSS过渡时，需要预置符合vue规则的带样式的类名，这些类名用于定义过渡不同阶段时的样式
    - v-enter
      定义进入过渡的开始状态，在元素被插入前生效，被插入后的下一帧移除
    - v-enter-active
      定义进入过渡生效时的状态，在整个进入过渡阶段中应用，在元素被插入之前生效，在过渡/动画完成之后移除，这个类可以用来定义进入过渡的过程时间、延迟和曲线函数等
    - v-enter-to
      定义进入过渡结束时的状态，在元素被插入后的下一帧生效(此时v-enter被移除)，在过渡/动画完成之后移除
    - v-leave
      定义离开过渡的开始状态，在离开过渡被触发时立刻生效，下一帧被移除
    - v-leave-active
      定义离开过渡生效时的状态，在整个离开过渡的阶段中应用，在离开过渡被触发时离开生效，在过渡/动画完成之后移除，这个类可以被用来定义离开过渡的过程时间’延迟和曲线函数等
    - v-leave-to
      定义离开过渡的结束状态，在离开过渡被触发之后的下一帧生效(此时v-leave被移除)，在过渡/动画完成之后移除
  - 当实例中存在多个不同的动画效果时，可以使用自定义前缀替换v-，比如使用slide-enter替换v-enter，不过帧这需要赋予transition元素name属性
  - 除了transition之外，还可以使用CSS中的animation，或直接使用第三方动画库(例如Animate.css)来实现过渡动画，由于这些动画库有着不同的类名规则，无法与vue默认的类名规则配合使用，因此vue提供了兼容方案，允许用户自定义过渡的类名，这些类名的优先级高于默认的类名，可以使用以下特性来自定义过渡类名
    - enter-class
    - enter-active-class
    - enter-to-class
    - leave-class
...
+ 使用JS实现过渡效果并不常见
**** 多节点过渡
+ 为列表元素或使用v-for渲染的元素添加过渡时，vue提供了transition-group组件，不同与transition的地方为
...
+ 示例
  ...
  #+END_SRC
+ RouterLink和RouterView是Vue Router提供的两个内置组件，RouterLink默认会被渲染一个<a>标签，它的to属性用于指定跳转链接，RouterView将负责挂载路由匹配到的视图组件
**** 动态路由
+ 示例
  #+BEGIN_SRC html
  <div id="app">
  ...
  <script>
      let Home = { template: '<h1>This is Home!</h1>' }
      let About = {
          template: '<div>' + '<h1>This is About!</h1>' + '<p>num: {{ $route.params.num }}</p>' + '</div>'
      }
      let routes = [
          { path: '/', component: Home },
  ...
  #+END_SRC
+ 可以使用动态路由参数将匹配某种模式的所有路由映射到同一个组件
+ 路径参数应用英文冒号:标记，但在使用时应注意设计的是否合理
**** 嵌套路由
+ 示例
  #+BEGIN_SRC html
  <div id="app">
  ...
  <script>
      let Home = { template: '<h1>This is Home!</h1>' }
      let About = {
          template: '<div>' + '<h1>This is About!</h1>' + '<router-view></router-view>' + '</div>'
      }
      let Author = { template: '<p>Author: Lonelydawn</p>' }
      let Emali = { template: '<p>Email: aaa@sina.com</p>' }
      let routes = [
          { path: '/', component: Home },
          { path: '/about', component: About,
            children: [
...
+ 多层的动态视图是可以使用vue内置组件component来实现的，但是component切换视图会在页面刷新后回到初始状态，而使用路由分发的视图在页面刷新后会保持当前路径对应的视图，并在浏览器的history中留下记录
**** 编程式路由
+ 示例
  #+BEGIN_SRC html
  <div id="app">
    ...
    let routes = [
        ...
        { 
          ...
          children: [
              ...
              { name: 'Email', path: 'emali/:email', component: Email }
          ]
        }
    ]
    let router = new VueRouter({
...
*** 状态管理
**** 简介
+ 对于组件化开发来讲,大型应用的状态往往跨越多个组件，在多层嵌套的父子组件之间传递状态是是十分麻烦的，而vue更是没有为兄弟组件提供直接共享数据的办法，Vuex就是vue官方提供保管所有分散的共享数据的状态管理器
+ 对于小型应用来说，完全没有必要引入状态管理，会带来更多的开发成本，然而当应用的复杂度很高时，状态管理会越发的重要
**** Vuex
+ Vuex用于管理分散在Vue各个组件中的数据
+ 每一个Vuex应用的核心都是一个store(仓库)，也可以想象为“非凡的全局对象”，与普通的全局对象不同的是，基于Vue数据与视图绑定的特点，当store中的状态发生变化时，与之绑定的视图也会被重新渲染
+ 这是一个单向的过程，store中的状态不允许被直接修改，改变store中的状态的唯一途径就是显示提交(commit)mutation，这可以方便的跟踪每一个状态的变化
**** Vuex重要概念
***** State
+ State用于维护所有应用层的状态，确保应用只有唯一的数据源
+ 语法
  #+BEGIN_SRC js
  new Vuex.Store({
      state: {
  ...
  #+END_SRC
+ 在组件中可以直接使用$store.state.count(前提是store已经被注册到实例中)
***** Getter
...
+ 语法
  ...
  #+END_SRC
+ 在组件中，可以直接使用$store.getters.tenTimesCount
***** Mutation
+ Mutation用来修改State状态
+ 语法
  #+BEGIN_SRC js
...
+ 在组件中，可以直接使用store.dispatch来分发action
  ...
  #+END_SRC
***** Module
+ 由于使用单一状态树，当项目的状态非常多时，store对象会变得非常臃肿，因此Vuex可以将store分割成模块(Module)，每个模块拥有独立的State、Getter、Mutation、Action，模块之中还可以嵌套模块，每一级都有着相同的结构
+ 语法
  #+BEGIN_SRC js
  //定义模块
  const counter = {
      ...
      actions: {
         //在模块中context还会具有rootState和rootGetters属性以获取根模块中的数据
         addCountAsync(context, num){
             setInterval(function (){
                 if (context.state.count < 2000) {
                     context.commint('addCount', num || 100)
                 }
             }, num || 100)
         }
      }
...
+ 在组件中，模块的使用方法如下
  ...
  #+END_SRC
***** 综述
+ 作为一个状态管理器，首先是保管状态的容器State
+ 为了满足衍生数据和数据链的需求，从而有了Getter
+ 为了显示修改状态，所有需要Mutation
+ 为了可以异步修改状态(满足AJAX等异步数据交互)，所以需要Action
+ 如果应用有成百上千个状态，放在一起会显得十分庞杂，所以需要模块管理(Module)
**** 在项目中使用Vuex
* 项目实例
** 
